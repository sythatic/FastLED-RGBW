...
void drawTwinkles(CRGB *leds) {
    ...
    if ((AUTO_SELECT_BACKGROUND_COLOR == 1) && (gCurrentPalette[0] == gCurrentPalette[1])) {
        ...
        uint8_t bglight = (bg.r + bg.g + bg.b) / 3;
        if (bglight > 64) {
            bg.r = (bg.r * 16) / 255;
            bg.g = (bg.g * 16) / 255;
            bg.b = (bg.b * 16) / 255;
    ...
    uint8_t backgroundBrightness = (bg.r + bg.g + bg.b) / 3;
    ...
    for (int i = 0; i < NUM_LEDS; i++) {
        ...
        uint8_t cbright = (c.r + c.g + c.b) / 3;
        int16_t deltabright = cbright - backgroundBrightness;
        if (deltabright >= 32 || (!bg.r && !bg.g && !bg.b)) {
            leds[i] = c;
